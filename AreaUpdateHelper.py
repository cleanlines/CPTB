...
class AreaUpdateHelper(Base):
    def __init__(self):
        ...
        self._area_fl = FeatureLayer(url=self._config.areaupdate["areaservice"], securityHandler=token.securityhandler,
                                     initialize=True)
        self._overlay_task = \
    ...
    def _process_overlay_results(self, layer, result_dic):
        ...
        if "featureSet" in result_dic:
            # # get FID field
            fd = \
                [k for k in result_dic["featureSet"]["features"][0]["attributes"].keys() if
                 "FID" in k and "AREA" not in k][0]
        ...
        update_features = layer.query(out_fields="OBJECTID," + self._config.areaupdate["updatefield"],
                                      returnGeometry=False, resultType='standard', objectIds=",".join(object_ids))
        ...
        for feat in result_dic["featureSet"]["features"]:
            ...
            if feat["attributes"][self._config.areaupdate["queryfield"]] is not None:
                updates[feat["attributes"][fd]] += \
                    feat["attributes"][self._config.areaupdate["queryfield"]].split(",")

        not_an_update = []
        for update_feature in update_features.features:
            if self._perform_area_update(update_feature.get_value(self._config.areaupdate["updatefield"]),
                                         updates[update_feature.get_value("OBJECTID")]):
                update_feature.set_value(self._config.areaupdate["updatefield"],
                                         ", ".join(updates[update_feature.get_value("OBJECTID")]))
            else:
                # same same
